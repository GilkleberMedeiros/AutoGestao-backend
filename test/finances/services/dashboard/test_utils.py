...
class TestDashboardService_ProjectsQS(TestCase):
  ...
  def test_projects_qs_filtering_and_optimization(self, mock_filter):
    user = MagicMock()
    period = PeriodFilterDTO(start_date=date(2026, 5, 1), end_date=date(2026, 5, 31))
    ...
    # Verify exclude Canceled projects
    mock_qs.exclude.assert_called_once_with(status=Project.CANCELLED_STATUS)
  ...
  def test_projects_qs_excludes_open_projects_when_requested(self, mock_filter):
    ...
    mock_qs.exclude.assert_has_calls(
      [
        call(status=Project.CANCELLED_STATUS),
        call(status=Project.OPEN_STATUS),
      ]
    )
...
class TestDashboardService_CalcProjectsBaseMetrics(TestCase):
  ...
  def test_calc_projects_base_metrics_caching_and_return_value(self, mock_projects_qs):
    user = MagicMock()
    period = PeriodFilterDTO(start_date=date(2026, 1, 1), end_date=date(2026, 1, 31))
