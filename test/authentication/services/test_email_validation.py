...
@patch("apps.authentication.services.email_validation.render_to_string")
@patch(
  "apps.authentication.services.email_validation.EmailValidationService._set_token"
)
@patch(
  "apps.authentication.services.email_validation.EmailValidationService._generate_token"
)
class TestEmailValidation_SendValidationEmail(TestCase):
  ...
  def test_send_validation_email_uses_defaults(
    ...
    self.assertEqual(render_args[0], EmailValidationService.validation_template)
  ...
  @patch(
    "apps.authentication.services.email_validation.EmailValidationService.delete_token"
  )
...
class TestEmailValidation_FormatToken(TestCase):
  def test_format_token(self):
    token = "token"
    formatted_token = EmailValidationService.format_token(token)
...
class TestEmailValidation_GenerateToken(TestCase):
  def test_generate_token(self):
    token = EmailValidationService._generate_token()

    self.assertIsInstance(token, str)
    self.assertNotEqual(token, "")


@patch("apps.authentication.services.email_validation.EmailValidationService.cache_db")
class TestEmailValidation_SetToken(TestCase):
  def test_set_token_donot_raises_exception(self, cache_db: MagicMock):
    ...
    try:
      EmailValidationService._set_token(user_id, token, timeout)
...
class TestEmailValidation_ValidateUserEmail(TestCase):
  ...
  @patch(
    "apps.authentication.services.email_validation.EmailValidationService._validate_token"
  )
  def test_validate_user_email_success(
    self, _validate_token: MagicMock, delete_token: MagicMock
  ):
    _validate_token.return_value = True

    result = EmailValidationService.validate_user_email(self.user, self.token)
  ...
  def test_validate_user_email_failure_db_error(self, _validate_token: MagicMock):
    ...
    with self.assertRaises(ExternalServiceError):
      EmailValidationService.validate_user_email(self.user, self.token)
  ...
  def test_validate_user_email_on_db(
    self, _validate_token: MagicMock, delete_token: MagicMock
  ):
    _validate_token.return_value = True
    ...
    EmailValidationService.validate_user_email(user, self.token)
...
class TestEmailValidation_ValidateToken(TestCase):
  @patch(
    "apps.authentication.services.email_validation.EmailValidationService.get_validation_data"
  )
  def test_validate_token_success(self, get_validation_data: MagicMock):
    ...
    result = EmailValidationService._validate_token(user_id, token)
...
class TestEmailValidation_GetValidationData(TestCase):
  @patch(
    "apps.authentication.services.email_validation.EmailValidationService.cache_db"
  )
  def test_get_validation_data_calls_cache_get(self, cache_db: MagicMock):
    token = "token"
    formatted_token = EmailValidationService.format_token(token)

    EmailValidationService.get_validation_data(token)
...
class TestEmailValidation_DeleteToken(TestCase):
  ...
  def test_delete_token_calls_cache_delete(self, cache_db: MagicMock):
    ...
    EmailValidationService.delete_token(token)
