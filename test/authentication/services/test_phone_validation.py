...
@patch("apps.authentication.services.phone_validation.render_to_string")
@patch(
...
class TestPhoneValidation_SendValidationSMS(TestCase):
  def setUp(self):
    self.user = MagicMock()
    self.user.id = "user_id"

  def test_send_validation_sms_success(
    self, generate_token: MagicMock, set_token: MagicMock, render_to_string: MagicMock
  ):
    token = "test-token"
    generate_token.return_value = token
    set_token.return_value = f"phone-validation-{token}"

    result = PhoneValidationService.send_validation_sms(self.user)
    self.assertEqual(result, token)
    generate_token.assert_called_once()
    set_token.assert_called_once_with(self.user.id, token)
    self.user.sms_user.assert_called_once()
    render_to_string.assert_called_once()

  def test_send_validation_sms_uses_defaults(
    self, generate_token: MagicMock, set_token: MagicMock, render_to_string: MagicMock
  ):
    PhoneValidationService.send_validation_sms(self.user)

    args, kwargs = self.user.sms_user.call_args
    # sms_user receives message as first arg and from_sms as kwarg
    self.assertEqual(kwargs.get("from_sms"), PhoneValidationService.validation_from_sms)
    render_to_string.assert_called_once()
    render_args, render_kwargs = render_to_string.call_args
    self.assertEqual(render_args[0], PhoneValidationService.validation_template)

  def test_send_validation_sms_uses_custom_values(
    self, generate_token: MagicMock, set_token: MagicMock, render_to_string: MagicMock
  ):
    custom_template = "custom/sms_template.txt"
    custom_from = "+5511999999999"

    PhoneValidationService.send_validation_sms(
      self.user,
      validation_template=custom_template,
      from_sms=custom_from,
    )

    args, kwargs = self.user.sms_user.call_args
    self.assertEqual(kwargs.get("from_sms"), custom_from)
    render_to_string.assert_called_once()
    render_args, render_kwargs = render_to_string.call_args
  ...
  def test_send_validation_sms_failure(
    self,
    ...
    self.user.sms_user.side_effect = Exception("SMS Error")
    formatted_token = "phone-validation-token"
